...
# -----------------------------------------------------------------------------
#' Registrera ett typsnitt för diagramrendering
#'
#' Registrerar ett typsnitt (via `showtext`/`sysfonts`) så att det faktiskt
#' renderas i PNG/SVG-export. Anropa en gång innan du ritar.
#'
#' @param namn Typsnittsnamn, t.ex. `"Poppins"`.
#' @param google Om `TRUE` hämtas typsnittet från Google Fonts (kräver internet).
#'
#' @return `TRUE` om typsnittet registrerades, annars `FALSE` (osynligt).
#' @export
...
# -----------------------------------------------------------------------------
#' Minsta omslutande cirkel för en mängd cirklar.
#' @keywords internal
#' @noRd
...
# -----------------------------------------------------------------------------
#' Snygga jämna skalvärden nära min-max.
#' @keywords internal
#' @noRd
...
# -----------------------------------------------------------------------------
#' Orm-/coil-layout för cirklar i storleksordning.
#' @keywords internal
#' @noRd
...
# -----------------------------------------------------------------------------
#' Bestäm geografisk omfattning för rubrik/filnamn.
#' @keywords internal
#' @noRd
bestam_omfattning <- function(data, layout_tabell = NULL, grupp_kol = NULL) {
  ...
  lan_etikett <- function(lannamn) {
    ...
    } else if (requireNamespace("rdverktyg", quietly = TRUE)) {
      rdverktyg::skapa_kortnamn_lan(lannamn)           # summerat -> "Dalarna"
    } else {
  ...
  if (length(koder) > 0 && requireNamespace("rdverktyg", quietly = TRUE)) {
    lan <- unique(substr(koder, 1, 2))
    if (length(lan) == 1) {
      alla <- tryCatch(rdverktyg::hamtakommuner(lan, tamedlan = FALSE, tamedriket = FALSE),
                       error = function(e) NULL)
      if (!is.null(alla) && length(alla) > 0 && all(alla %in% koder)) {
        lannamn <- tryCatch(rdverktyg::hamtaregion_kod_namn(lan)$region,
                            error = function(e) NULL)
...
#' Hämta (och cacha) branschnyckelfilen.
#' @keywords internal
#' @noRd
...
#' Packed-circle-diagram: sysselsatta per bransch
#'
#' Ritar circle-packing-diagram per grupp (kommun, kön eller hela regionen)
#' enligt Region Dalarnas profil. Layout väljs automatiskt: Dalarnas kommuner
#' -> geografisk placering, uppdelning på kön/födelseregion -> rutnät.
#'
#' @param data En `data.frame` i långt format (en rad per grupp + bransch).
#' @param grupp_kol kolumnnamn for gruppering, t.ex. "region", "kon" eller "fodelseregion". NULL (default) = en enda grupp.
#' @param bransch_kol kolumnnamn for branschnamn (anvands i legend); NULL = auto-detektera (se AVANCERAT for de kolumnlistor autodetekteringen anvander)
#' @param antal_kol kolumnnamn for antal sysselsatta; NULL = auto-detektera (se AVANCERAT for de kolumnlistor autodetekteringen anvander)
#' @param farg_kol kolumnnamn med hex-farg per rad; NULL = auto-palett
#' @param diagram_caption valfri bildtext/kalla under diagrammet; NULL = ingen
#' @param titel plot-titel som strang, eller NULL
#' @param bakgrund bakgrundsfarg; "black" ger ljus text och svart panel
#' @param visa_antal TRUE/FALSE = skriv ut siffror i cirklarna alls
#' @param antal_min minsta antal for utskriven siffra; NULL = autoberakna fran cirkelstorlek (se dokumentation ovan)
#' @param visa_namn TRUE/FALSE = visa gruppnamn vid klungorna
#' @param autokoppla_branschnamn_farg TRUE = upptack branschkod-kolumn (Gxx m.m.) och koppla pa Bransch + HexCode fran nyckelfilen automatiskt
#' @param skal_bubblor 0 = inga; >0 = antal svarta skalbubblor (storleksreferens)
#' @param diskreta_labels TRUE = sifferfarg = nyans av bubblans egen farg; FALSE = svart/vit
#' @param labels_visa_skalvarden TRUE = visa siffror BARA i cirklarna narmast skalvardena (kraver skal_bubblor > 0); FALSE = enligt antal_min
#' @param storlek_titel textstorlek for plottiteln (ggplot2 size-enhet)
#' @param storlek_caption textstorlek for diagram_caption (ggplot2 size-enhet)
#' @param storlek_legend textstorlek for legendtext i pt
...
#' @param bildhojd bildhojd i tum; bredden beraknas dynamiskt om bildbredd = NULL
#' @param layout "auto" = valj sjalv (se ovan)
#' @param storlek_ut TRUE = storsta cirklarna i ytterkanten; FALSE = i mitten
#' @param packning "spiral" = orm-coil i storleksordning; "progressiv" = tatast
#' @param visa_ring TRUE/FALSE = rita tunn ring runt varje klunga
#' @param ring_farg farg pa ringen
#' @param ring_marginal ringradie / klungans yttre kant; 1.00-1.10
#' @param ring_metod "omslutande" = tatare ring; "origo" = fran spiralens origo
#' @param geo_metod layout="geo": "skala_bubblor" = trogen karta
#' @param kompaktera layout="geo": TRUE = dra ihop kommunerna (tatare karta)
#' @param layout_tabell data.frame med kolumner grupp, gx, gy (0-100, y uppat)
#' @param start_grupp layout="vinkel": namn pa centrum-grupp; NULL = storsta
#' @param ncol antal kolumner i layout="grid"
#' @param skala_grupp TRUE = klungans storlek beror pa gruppens total
#' @param skala_styrka 0-1: 1.0 full proportion, lagre = dampad; NULL (default) = autoberakna fran spridningen mellan grupperna (se dokumentation ovan)
#' @param tathet avstandsfaktor: <1 tatare, >1 glesare
#' @param hojd_andel 0-1: andel av panelhojd diagrammet upptar; NULL (default) = berakna exakt utrymme for titel/caption (se dokumentation ovan)
#' @param namn_utanfor TRUE = gruppnamn UTANFOR ringen
#' @param font typsnittsnamn
#' @param autoladda_typsnitt TRUE = registrera 'font' automatiskt, annars standardfont
#' @param legend_radbryt max tecken per rad i legendtext; 0/NULL = ingen brytning
#' @param dpi upplosning vid sparande
#' @param bildbredd bildbredd i tum; NULL = beraknas fran innehallets proportioner (minskar vit luft)
#' @param bransch_nyckel_url nyckelfil for autokoppla_branschnamn_farg
#' @param bransch_kol_kandidater bransch_kol=NULL: exakta kolumnnamn (case-insensitive) att leta efter forst. Lagg till fler har.
#' @param bransch_kol_monster bransch_kol=NULL: hittas ingen exakt traff, leta efter kolumn vars namn innehaller ALLA strangar i nagon delmangd (OR mellan delmangder, case-insensitive). L...
#' @param bransch_kol_kod_monster bransch_kol=NULL: vid FLERA traffar, ta bort de vars namn tyder pa KOD (bransch_kol ska vara klartext - anvands direkt som legendtext). Meddelas alltid i kon...
#' @param antal_kol_kandidater antal_kol=NULL: exakta kolumnnamn (case-insensitive) att leta efter forst. Lagg till fler har.
#' @param antal_kol_monster antal_kol=NULL: hittas ingen exakt trakk, leta efter kolumn vars namn innehaller ALLA strangar i nagon delmangd (OR mellan delmangder). Lagg till fler delman...
#' @param antal_kol_prioritet antal_kol=NULL: vid FLERA traffar, foredra en kolumn som innehaller nagot av dessa (case-insensitive). Meddelas alltid i konsolen nar detta avgor valet.
#'
...
skapa_packed_circles <- function(data,
                                 ...
                                 layout_tabell = NULL,                 # data.frame med kolumner grupp, gx, gy (0-100, y uppat)
    ...
    if (!requireNamespace("curl", quietly = TRUE) || !requireNamespace("readxl", quietly = TRUE)) {
      ...
      if (is.null(nyckel)) {
        ...
        if (!is.null(basta_datakol) && basta_andel >= 0.5) {
          ...
          bransch_map <- stats::setNames(lk$Bransch, as.character(lk[[basta_kodkol]]))
          farg_map    <- stats::setNames(lk$HexCode,  as.character(lk[[basta_kodkol]]))
  ...
  if (all(is.na(df$.farg))) {
    ...
    palett <- stats::setNames(scales::hue_pal()(length(bran)), bran)
  } else {
    pp <- dplyr::distinct(df, .bransch, .farg)
    palett <- stats::setNames(pp$.farg, pp$.bransch)
  }
  ...
  for (g in grupper) {
    ...
    if (packning == "spiral") {
      ...
      lay <- packcircles::circleProgressiveLayout(sub$.antal, sizetype = "area")
    }
  ...
  if (skala_grupp) {
    ...
    klung_r <- stats::setNames(rep(1, length(totals)), names(totals))
  }
  ...
  if (layout == "geo") {
    ...
    if (geo_metod == "skala_bubblor") {
      ...
      # Vektoriserat med stats::dist()/outer() istallet for dubbel for-loop (snabbare, och
      # undviker R:s "1:(n-1)"-sekvensfalla som kraschade nar n == 1).
      if (n >= 2) {
        d_mat <- as.matrix(stats::dist(cbind(pos$gx, pos$gy)))
    ...
    if (n >= 2) {
      for (iter in 1:3000) {
        ...
        if (!moved) {
          ...
          # koll med stats::dist()/outer() istallet for dubbel for-loop
          d_mat <- as.matrix(stats::dist(cbind(px, py)))
          Rsum  <- outer(R, R, "+")
    ...
    OX <- stats::setNames(numeric(nrow(pos)), pos$.gid)
    OY <- stats::setNames(numeric(nrow(pos)), pos$.gid)
  ...
  for (g in grupper) {
    ...
    poly  <- packcircles::circleLayoutVertices(lay_g, npoints = 60)
  ...
  p <- p +
    ggplot2::geom_polygon(data = polygoner, ggplot2::aes(x, y, group = .uid, fill = .bransch),
                 colour = NA) +
    ggplot2::scale_fill_manual(values = palett, name = NULL, breaks = legend_ordning,
                      labels = legend_etiketter) +
    ggplot2::theme_void(base_family = font) +
    ggplot2::theme(plot.background  = ggplot2::element_rect(fill = bakgrund, colour = NA),
          panel.background = ggplot2::element_rect(fill = bakgrund, colour = NA),
          legend.text  = ggplot2::element_text(colour = txt_default, size = storlek_legend),
          legend.key.size = ggplot2::unit(storlek_legend * 0.06, "cm"),
  ...
  if (!is.null(titel)) {
    ...
    p <- p + ggplot2::annotate("text", x = kx_mitt, y = titel_y, label = titel,
  ...
  if (!is.null(diagram_caption)) {
    ...
    p <- p + ggplot2::annotate("text", x = x_min, y = caption_y, label = diagram_caption,
  ...
  if (skal_bubblor > 0 && !is.null(skal_varden)) {
    ...
    p <- p +
      ggforce::geom_circle(data = skal_df, ggplot2::aes(x0 = x, y0 = y, r = r),
  ...
  if (visa_antal || visa_labels_vid_skala) {
    ...
    if (diskreta_labels) {
      ...
      rgbm <- grDevices::col2rgb(cirklar$.farg)
      ...
      cirklar$.txt <- grDevices::rgb(ny_mat[1,], ny_mat[2,], ny_mat[3,], maxColorValue = 255)
    } else {
      rgbm <- grDevices::col2rgb(cirklar$.farg)
      lum  <- (0.299*rgbm[1,] + 0.587*rgbm[2,] + 0.114*rgbm[3,]) / 255
      cirklar$.txt <- ifelse(lum < 0.5, "white", "black")
    }
    p <- p + ggplot2::geom_text(data = cirklar, ggplot2::aes(x, y, label = .lbl),
  ...
  if (visa_namn && !is.null(grupp_kol)) {
    ...
    p <- p + ggplot2::geom_text(data = namn, ggplot2::aes(x, y, label = .gid),
  ...
  p <- p + ggplot2::coord_equal(xlim = xl, ylim = yl, clip = "off")
  ...
  if (isTRUE(spara_bildfil)) {
    # 1. Mapp: angiven -> annars rdverktyg::utskriftsmapp() -> annars fel
    if (is.null(mapp)) {
      if (requireNamespace("rdverktyg", quietly = TRUE)) {
        mapp <- rdverktyg::utskriftsmapp()
      } else {
        stop("Ingen mapp angiven. Ange 'mapp = ...' eller installera 'rdverktyg' ",
             "(rdverktyg::utskriftsmapp()).", call. = FALSE)
      }
    }
    ...
    ggplot2::ggsave(full_sokvag, p, width = bb, height = bh, dpi = dpi, bg = bakgrund)
...
# -----------------------------------------------------------------------------
#' Förhandsvisa ett diagram i samma proportioner som den sparade bildfilen
#'
#' @param p Ett ggplot-objekt (helst med attributet `bildmatt`).
#' @param bredd,hojd Mått i tum, eller `NULL` för att läsa från `bildmatt`.
#'
#' @return `p` (osynligt).
#' @export
forhandsvisa <- function(p, bredd = NULL, hojd = NULL) {
  ...
  grDevices::dev.new(width = bredd, height = hojd, unit = "in", noRStudioGD = TRUE)
  print(p)
  invisible(p)
}